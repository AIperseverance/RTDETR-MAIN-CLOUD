...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 C2f_MAC, C2f_TransMixer, C2f_DEGConv, C2f_GLSS2D, C2f_SPJFB, C2f_MSInit, C2f_PFG, C2f_CKConv, C2f_RFGM,
                 C2f_IRA, C2f_PatchMamba, C2f_GradMamba, C2f_GLCDM, C2f_SMS, C2f_SMB,
                 C2f_DSEBlock, C2f_LaSEA, C2f_SFEB, C2f_FourierSR, C2f_FrequencyCM):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, DWRC3, C3_DWR, C2f_DWR, C3_DCNv2_Dynamic, C2f_DCNv2_Dynamic,
                     ...
                     C2f_RMBC, C2f_RMBC_LA, C2f_SFMB, C2f_MFEB, C2f_PartialNetBlock, C2f_DRG, C2f_GLGM, C2f_MAC,  C2f_TransMixer, C2f_DEGConv, C2f_GLSS2D, C2f_SPJFB, C2f_MSInit, C2f_PFG, C2f_CKConv, C2f_RFGM,
                     C2f_IRA, C2f_PatchMamba, C2f_GradMamba, C2f_GLCDM, C2f_SMS, C2f_SMB,
                     C2f_DSEBlock, C2f_LaSEA, C2f_SFEB, C2f_FourierSR, C2f_FrequencyCM):
                ...
                n = 1
                   ...
                   TransformerEncoderLayer_BinaryAttn, TransformerEncoderLayer_WCA, TransformerEncoderLayer_LCGA, TransformerEncoderLayer_CGTA, AIFI_AFFN, TransformerEncoderLayer_AFFN,
                   TransformerEncoderLayer_CirculantAttn, TransformerEncoderLayer_WDAM):
            c2 = ch[f]
            args = [ch[f], *args]
            ...
            args = [c1, args[0]]
        elif m in (RTDETRDecoder, RTDETRDecoder_SAQE, RTDETRDecoder_MSAQE, RTDETRDecoder_GatedScaleSAQE):  # special case, channels arg must be passed in index 1
        ...
        # --------------ASF--------------
            ...
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [c1, c2]
        elif m in {MFPM, ERM, CAFM_Fusion, SFSFusion, FAAFusion, HAFFormer}:
            c1 = [ch[x] for x in f]
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        else:
