...
__all__ = ['TransformerEncoderLayer_LocalWindowAttention', 'AIFI_LPE', 'TransformerEncoderLayer_DAttention', 'TransformerEncoderLayer_HiLo', 
           ...
           'AIFI_AFFN', 'TransformerEncoderLayer_AFFN', 'TransformerEncoderLayer_CirculantAttn', 'TransformerEncoderLayer_WDAM']
...
class TransformerEncoderLayer_CirculantAttn(nn.Module):
    """Defines a single layer of the transformer encoder."""

    def __init__(self, c1, cm=2048, num_heads=8, dropout=0.0, act=nn.GELU(), normalize_before=False):
        super().__init__()
        self.SWSA = CirculantAttention(c1)
        self.fc1 = nn.Conv2d(c1, cm, 1)
        self.fc2 = nn.Conv2d(cm, c1, 1)
...
class TransformerEncoderLayer_WDAM(nn.Module):
    """Defines a single layer of the transformer encoder."""

    def __init__(self, c1, cm=2048, num_heads=8, dropout=0.0, act=nn.GELU(), normalize_before=False):
        super().__init__()
        self.SWSA = WDAM(c1, num_heads=num_heads)
        self.fc1 = nn.Conv2d(c1, cm, 1)
        self.fc2 = nn.Conv2d(cm, c1, 1)
...
class TransformerEncoderLayer_AFFN(nn.Module):
    def __init__(self, c1, cm=2048, num_heads=8, dropout=0.0, act=nn.GELU(), normalize_before=False):
        super().__init__()
        ...
        if not TORCH_1_9:
            raise ModuleNotFoundError(
                "TransformerEncoderLayer_AFFN() requires torch>=1.9 to use nn.MultiheadAttention(batch_first=True)."
            )
        self.ma = nn.MultiheadAttention(c1, num_heads, dropout=dropout, batch_first=True)
        self.ffn = AFFN(c1, c1 * 2, c1)
    ...
    def forward_post(self, src, src_mask=None, src_key_padding_mask=None, pos=None):
        ...
        src = src + self.dropout1(src2)
        src = self.norm1(src)
        src2 = self.ffn(src.permute(0, 2, 1).view([b, c, h, w]).contiguous()).flatten(2).permute(0, 2, 1)
        src = src + self.dropout2(src2)
        return self.norm2(src)
...
class AIFI_AFFN(TransformerEncoderLayer_AFFN):
    def __init__(self, c1, cm=2048, num_heads=8, dropout=0, act=nn.GELU(), normalize_before=False):
        super().__init__(c1, cm, num_heads, dropout, act, normalize_before)
